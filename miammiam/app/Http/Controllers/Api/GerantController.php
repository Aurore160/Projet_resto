...
class GerantController extends Controller
{
    ...
    /**
     * Lister toutes les commandes (pour le gérant)
     * 
     * GET /api/gerant/orders
     * 
     * Accessible uniquement aux gérants
     * Retourne toutes les commandes (exclut les paniers) avec les détails des articles
     * 
    ...
    public function listOrders(Request $request)
    {
        try {
            ...
            $limit = $request->query('limit', 50); // Limite par défaut pour éviter les timeouts
            $offset = (int) ($request->query('offset', 0));

            // Utiliser Eloquent pour de meilleures performances
            $query = Commande::with(['utilisateur', 'articles.menuItem'])
                ->where('statut', '!=', 'panier');
            ...
            $commandes = $query->skip($offset)->take($limit)->get();

            // Formater les données
            $commandesAvecArticles = $commandes->map(function ($commande) {
                ...
                $articlesFormates = $commande->articles->map(function ($article) {
                    return [
                        ...
                        'plat' => [
                            ...
                            'description' => $article->menuItem->description ?? '',
                ...
                return [
                    ...
                    'nb_articles' => (int) $commande->articles->sum('quantite'),
                    'articles' => $articlesFormates,
                    'utilisateur' => $commande->utilisateur ? [
                        'id_utilisateur' => (int) $commande->utilisateur->id_utilisateur,
                        ...
                        'photo' => $commande->utilisateur->photo ?? null,
                    ] : null,
                ];
            })->toArray();
            ...
            \Log::error('Erreur lors de la récupération des commandes (gérant)', [
