...
class NotificationController extends Controller
{
    ...
    public function index(Request $request)
    {
        try {
            ...
            $this->logger->info('Récupération des notifications', [
            ...
            $notificationsFormatees = $notifications->map(function ($notification) {
                // Formatage sécurisé des dates (les casts garantissent que c'est Carbon ou null)
                $dateCreation = null;
                if ($notification->date_creation) {
                    try {
                        $dateCreation = $notification->date_creation->format('Y-m-d H:i:s');
                    } catch (\Exception $e) {
                        // Si erreur de formatage, utiliser la valeur brute ou null
                        $dateCreation = is_string($notification->date_creation) ? $notification->date_creation : null;
                        $this->logger->warning('Erreur formatage date_creation', [
                            'notification_id' => $notification->id_notification,
                            'error' => $e->getMessage(),
                            'date_value' => $notification->date_creation,
                        ]);
                    }
                }
                ...
                if ($notification->date_lecture) {
                    try {
                        ...
                        $this->logger->warning('Erreur formatage date_lecture', [
                            'notification_id' => $notification->id_notification,
                            'error' => $e->getMessage(),
                            'date_value' => $notification->date_lecture,
                        ]);
                    }
                }
            ...
            $this->logger->error('Erreur dans NotificationController::index', [
                'error' => $e->getMessage(),
                'file' => $e->getFile(),
                'line' => $e->getLine(),
     ...
     *   "message": "Nous avons une réunion demain à 10h dans la salle de conférence.",
     *   "type_notification": "system",
     *   "id_employes": [1, 2, 3] // Optionnel : si non fourni, envoie à tous les employés actifs
     * }
     */
            ...
            foreach ($employes as $employe) {
                try {
                    $notification = Notification::create([
                        ...
                        'id_commande' => null, // Pas de commande associée pour les notifications manuelles
